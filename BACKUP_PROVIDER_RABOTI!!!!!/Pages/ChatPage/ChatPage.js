...
var userObj;
var pch;
var p_isPatient = Observable(true);
...
this.onParameterChanged(function(param) {
    ...
    if (param.doctorChatRoomId) {
        ...
        if (((JSON.parse(user.value.name)).isPatient) == "1") {
            panelVisibility.value = "Visible";
            p_isPatient.value = true;
        } else {
            panelVisibility.value = "Collapsed";
            p_isPatient.value = false;
        }
    ...
    if (panelVisibility.value == "Visible") {
        ...
        var url = activeUrl.URL + "/curandusproject/webapi/api/getPatientsDataByChatId/chatid=" + patientchatid.value;
        fetch(url, {
            method: 'GET',
            headers: {
        ...
        var url = activeUrl.URL + "/curandusproject/webapi/api/GetProviderDataByChatId/chatid=" + patientchatid.value;
        fetch(url, {
            method: 'GET',
            headers: {
                "Content-type": "application/json",
...
function goToTreatment() {
    ...
    if (!(parametar_send.value)) {
        var url = activeUrl.URL + "/curandusproject/webapi/api/treatment/roomid=" + RoomId;
