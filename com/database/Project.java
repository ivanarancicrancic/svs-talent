...
//	 } 	
	 ...
	 public VerificationToken generateSecurityToken(Connection connection) throws SQLException, ParseException {
			String token = UUID.randomUUID().toString(); 
	    	VerificationToken new_token = null;
	    	PreparedStatement preparedStatement1=null;
	    	try {
	          new_token = createVerificationToken(token);
	    	 System.out.println("Generated token: " + token + ", with expiryDate: " + new_token.getExpiryDate());
		         String insertTableSQL = "";
			     	System.out.println("New token is generated and will be inserted into verificationtoken.");
			         //insertTableSQL = "INSERT INTO verificationtoken (Token, ProviderID, ExpiryDate, ExpiryTime) VALUES (?, ?, ?, ?)";
			         insertTableSQL = "INSERT INTO verificationtoken (Token, ExpiryDate, ExpiryTime) VALUES (?, ?, ?)"; 
			         PreparedStatement preparedStatement= connection.prepareStatement(insertTableSQL);
			         preparedStatement.setString(1, new_token.getToken());
			         java.util.Date utilDate = new_token.getExpiryDate();
			         java.sql.Date sqlDate3 = new java.sql.Date(utilDate.getTime());
			         preparedStatement.setDate(2, sqlDate3);
			         preparedStatement.setTime(3, new_token.getTime());
			        preparedStatement.executeUpdate();
			            
			        PreparedStatement preparedStatement123= connection.prepareStatement("SELECT * FROM verificationtoken");
			        ResultSet result = preparedStatement123.executeQuery();
			        
			        while(result.next())
			        {
			        Calendar cal = Calendar.getInstance();
					Calendar cal1 = Calendar.getInstance();
					cal1.setTime(result.getDate("ExpiryDate"));
					...
					Time expirationTime = result.getTime("ExpiryTime");
...
//						System.out.println("Left seconds until expiration:" + leftSeconds);
						preparedStatement1 = connection.prepareStatement("DELETE FROM verificationtoken WHERE verificationtoken.Token='" + result.getString("Token") + "'");
						if(expirationDay - currentDay < 0) {
							  preparedStatement1.executeUpdate();	 
							 System.out.println("Token is deleted due to day expiration!");
						  continue;
						...
						{   
							preparedStatement1.executeUpdate();	
							System.out.println("Token is deleted due to hour expiration!");
							continue;
						//	throw new Exception("Token invalid due to hour expiration!");	
							...
							preparedStatement1.executeUpdate();
							System.out.println("Token is deleted due to minute expiration!");
							continue;
							...
							preparedStatement1.executeUpdate();
							System.out.println("Token is deleted due to seconds expiration!");
							continue;
							//throw new Exception("Token invalid due to seconds expiration!");	
						}
						   
			        }	
			        System.out.println("Expired tokens are removed from database!");
	 ...
	 public boolean checkSecurityToken(Connection connection, String securityToken) throws Exception {
		...
		preparedStatement= connection.prepareStatement(checkSecurityTokenSQL);    
	     ...
	     if (rs2.next()){
	       ...
	       //found_token.setProvider_id(rs2.getInt("ProviderID"));
...
//				ResultSet rs = ps.executeQuery();	
	...
	public List<ReminderNotification> GetNotificationForReminder(Connection connection, String securityToken) throws Exception
	{
		...
		PreparedStatement ps=null;
			try
			{
				...
				if (t_items==null || t_items.isEmpty()){
					 throw new WebApplicationException(404);
				}
				else
				{
	...
	public boolean SendSMS(Connection connection, String To, String Body, String securityToken) throws Exception{
        boolean flag=false;
		try {
			 if(checkSecurityToken(connection, securityToken)==false) {
				 throw new WebApplicationException(404);	 
...
//	 }
	...
	public Providers InsertProvider(Connection connection, Providers p_provider, String securityToken) throws Exception
	 {
	  PreparedStatement ps=null;
	  Providers result=new Providers();
	  try
...
//	}
  ...
  VerificationToken myToken=null;
	try {
		...
		System.out.println(token);
//		myToken = new VerificationToken(token, provider.getProviderId());
		myToken = new VerificationToken(token);
