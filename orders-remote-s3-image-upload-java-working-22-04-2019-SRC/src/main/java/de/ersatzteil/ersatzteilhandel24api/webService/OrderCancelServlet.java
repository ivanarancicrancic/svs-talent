...
@WebServlet("/ersatzteilhandel24apiValid/OrderCancel")
public class OrderCancelServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        JsonNode orderJsonNode = jsonNode.get("order_to_cancel");
        if (orderJsonNode != null) {
            String order_id = (orderJsonNode.get("order_id").asText());
            String cancelReason = (orderJsonNode.get("cancelReason").asText());
            Order order_item = new Order();
            try{
                ProjectManager projectManager= new ProjectManager();
                order_item = projectManager.cancelOrder(order_id, cancelReason);
